...
`Tools/quick-clipboard-test.swift` checks light/dark semantic label contrast, selection emphasis, fixed-size symbols, the 120-point width cap, stationary keyboard selection and history activation, editable web roles, numeric/marker caret fallback and nonempty-selection rejection, caret priority, mouse fallback, cross-display coordinate conversion, above/below placement, untinted Clear glass content containment, borderless button click routing, the unified area shadow and 1×/2× cutout pixels for the complete group,
