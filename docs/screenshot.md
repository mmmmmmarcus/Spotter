...
The selection view replaces the system pointer for the bounded session with the supplied 35×35
Union.svg cursor, then restores the exact previous cursor on every exit path. Its unoutlined
`#2076FF` source shape is rendered from compiled CoreGraphics geometry into the AppKit cursor once per
panel, then receives a one-point external white outline generated by Spotter. A two-point, 28%-black
drop shadow sits one point below the artwork; four points of transparent
cursor padding on every side keep the blur unclipped without moving its hotspot. There is no runtime
SVG parsing or second overlay-drawn pointer. Cursor rects cover each display and the cursor is set
on presentation, every inactive-panel mouse move and explicitly during a drag, so Option-key release
and the previous app's cursor rect cannot reintroduce the arrow before or after mouse-down. A
