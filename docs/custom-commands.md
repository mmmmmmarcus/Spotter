...
The first store migration adds two ordinary, editable starter commands with stable UUIDs:
`Sleep Display` (`/usr/bin/pmset displaysleepnow`) and `Toggle Dark / Light Mode` (an `osascript`
command that flips System Events' appearance preference). The migration marker is written separately
from the command array, so editing or deleting a starter is respected and it never respawns. A
pre-existing command with the same UUID, case-insensitive name or exact shell text wins instead of
...
containing both `CommandRegistry` and custom commands, including the starters. This keeps the visible
row order identical to the flat palette selection while allowing edits to invalidate fuzzy results
