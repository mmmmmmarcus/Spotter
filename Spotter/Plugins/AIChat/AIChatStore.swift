...
struct AIChatSession: Identifiable, Equatable, Sendable {
    ...
    init(id: UUID = UUID(), messages: [AIChatMessage] = [], startedAt: Date = Date()) {
        self.id = id
        self.messages = messages
        self.startedAt = startedAt
    }

    var title: String { AIChatEngine.sessionTitle(for: messages) }
}
...
final class AIChatStore: ObservableObject {
    ...
    func startNewSession() {
        ...
        // Drop empty leftovers; the menu should only list conversations that happened.
        sessions.removeAll { $0.messages.isEmpty }
        let fresh = AIChatSession()
    ...
    /// Appends the turn and asks; returns false when nothing could be sent (empty, busy, or no key).
    @discardableResult
    func send(_ text: String) -> Bool {
        ...
        task = Task { [weak self] in
            ...
            do {
                let turns =
                    [(role: "system", content: AIChatEngine.systemPrompt)]
                    + window.map { (role: $0.role.rawValue, content: $0.text) }
                let reply = try await self.openRouter.chat(
                    messages: turns, model: self.openRouter.chatModel,
                    webSearch: self.openRouter.chatWebSearch)
