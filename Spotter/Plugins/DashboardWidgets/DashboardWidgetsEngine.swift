...
enum DashboardWidgetsEngine {
    ...
    /// What the clock alone is running on — the whole answer for a Mac that has not granted weather,
    /// since nothing is located for it. Never empty: a clock with no saved zone names the system's.
    static func clockSummary(clockTimeZoneIdentifier: String?, systemTimeZoneIdentifier: String)
        -> String
    {
