...
struct NoteTests {
    ...
    static func main() async {
        ...
        check("window transparency snaps to medium", 0.45, store.windowTransparency)
        var titleChanges = 0
        store.onTitlesChanged = { titleChanges += 1 }
        store.updateSelectedContent("# Persisted\nBody")
        check("first title enters the launcher index", 1, titleChanges)
        store.updateSelectedContent("# Persisted\nEdited body")
        check("body editing leaves the title index alone", 1, titleChanges)
        store.updateSelectedContent("# Renamed\nBody")
        check("renaming refreshes the title index", 2, titleChanges)
        store.updateSelectedContent("# Persisted\nBody")
        check("restored title refreshes the index", 3, titleChanges)
        check("seven selectable tints", 7, NoteTint.selectable.count)
        check("three transparency presets", 3, NoteTransparency.allCases.count)
        await store.flush()
        ...
        check("window transparency persists", 0.45, reopened.windowTransparency)
